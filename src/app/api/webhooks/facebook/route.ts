...
async function handleMessage(pageId: string, event: any) {
    ...
    if (referral) {
        // referral.ad_id - ID à¸‚à¸­à¸‡à¹‚à¸†à¸©à¸“à¸²
        // referral.ref - custom ref à¸—à¸µà¹ˆà¸•à¸±à¹‰à¸‡à¹„à¸§à¹‰ (à¸–à¹‰à¸²à¸¡à¸µ)
        // referral.source - à¹à¸«à¸¥à¹ˆà¸‡à¸—à¸µà¹ˆà¸¡à¸² à¹€à¸Šà¹ˆà¸™ ADS
        // referral.type - à¸›à¸£à¸°à¹€à¸ à¸— à¹€à¸Šà¹ˆà¸™ OPEN_THREAD
        adId = referral.ad_id || null;
        console.log(`[Webhook] Message from Ad! Ad ID: ${adId}, Source: ${referral.source}, Ref: ${referral.ref}`);

        // à¸–à¹‰à¸²à¸•à¹‰à¸­à¸‡à¸à¸²à¸£à¸”à¸¶à¸‡à¸Šà¸·à¹ˆà¸­à¹‚à¸†à¸©à¸“à¸² à¸ªà¸²à¸¡à¸²à¸£à¸–à¹€à¸£à¸µà¸¢à¸ Marketing API à¹„à¸”à¹‰
        // à¹à¸•à¹ˆà¸•à¹‰à¸­à¸‡à¸¡à¸µ ads_read permission - à¸•à¸­à¸™à¸™à¸µà¹‰à¹€à¸à¹‡à¸šà¹à¸„à¹ˆ ad_id à¸à¹ˆà¸­à¸™
    }
    ...
    if (message.attachments && message.attachments.length > 0) {
        ...
        if (!messageContent) {
            ...
            if (firstAtt.type === 'sticker') {
                ...
                messageContent = '[à¸£à¸¹à¸›à¸ à¸²à¸ž]';
            } else if (firstAtt.type === 'video') {
                messageContent = '[à¸§à¸´à¸”à¸µà¹‚à¸­]';
            } else if (firstAtt.type === 'audio') {
                messageContent = '[à¹€à¸ªà¸µà¸¢à¸‡]';
            } else if (firstAtt.type === 'file') {
                messageContent = '[à¹„à¸Ÿà¸¥à¹Œ]';
    ...
    if (!isFromPage) {
        ...
        // INVALIDATE CACHE if it contains "à¸¥à¸¹à¸à¸„à¹‰à¸²"
        if (cachedName && (cachedName.name === 'à¸¥à¸¹à¸à¸„à¹‰à¸²' || cachedName.name === 'Customer')) {
            ...
            // Treat 'à¸¥à¸¹à¸à¸„à¹‰à¸²' as a legacy/invalid name that should be overwritten
            ...
            const isLegacyName = !existingConv?.participantName ||
                existingConv.participantName === 'à¸¥à¸¹à¸à¸„à¹‰à¸²' ||
                ...
                try {
                    ...
                    if (pageToken) {
                        ...
                        if (userData.name) {
                            ...
                            if (existingConv?.participantName && existingConv.participantName !== 'à¸¥à¸¹à¸à¸„à¹‰à¸²') {
                                senderName = existingConv.participantName;
                                participantName = existingConv.participantName;
                            }
                        }
    ...
    try {
        ...
        if (existingConversation && existingConversation.id) {
            ...
            // Real conversation ID à¸ˆà¸°à¸–à¸¹à¸ sync à¹€à¸¡à¸·à¹ˆà¸­ user à¸à¸” manual sync
        ...
        // Also update if the current name is 'à¸¥à¸¹à¸à¸„à¹‰à¸²' (legacy)
        ...
        await db.upsertConversation(conversationId,
            // create data (à¸–à¹‰à¸²à¹€à¸›à¹‡à¸™ conversation à¹ƒà¸«à¸¡à¹ˆ)
            {
                pageId: pageId,
                lastMessageAt: new Date(),
                snippet: messageContent,
                unreadCount: isFromPage ? 0 : 1, // à¸–à¹‰à¸²à¹€à¸£à¸²à¸ªà¹ˆà¸‡ = 0, à¸–à¹‰à¸²à¸¥à¸¹à¸à¸„à¹‰à¸²à¸ªà¹ˆà¸‡ = 1
                participantId: otherUserId,
                participantName: participantName,
                adId: adId
            },
            // update data (à¸–à¹‰à¸²à¸¡à¸µ conversation à¸­à¸¢à¸¹à¹ˆà¹à¸¥à¹‰à¸§ - à¹„à¸¡à¹ˆà¸­à¸±à¸žà¹€à¸”à¸— unreadCount à¸—à¸µà¹ˆà¸™à¸µà¹ˆ)
            {
                lastMessageAt: new Date(),
                snippet: messageContent,
        ...
        // Increment unread count à¹€à¸‰à¸žà¸²à¸°à¸‚à¹‰à¸­à¸„à¸§à¸²à¸¡à¸ˆà¸²à¸à¸¥à¸¹à¸à¸„à¹‰à¸² (à¹„à¸¡à¹ˆà¹ƒà¸Šà¹ˆà¸ˆà¸²à¸à¹€à¸£à¸²)
        if (!isFromPage) {
